// Web Audio API lightweight sound effects synthesizer (zero external assets needed)
...
// 1. Soft Pop Sound (When sending a message)
...
// 2. Gentle Message Receive Sound
...
export function playReactionSound() {
  try {
    ...
    notes.forEach((freq, i) => {
      ...
      gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + i * 0.04 + 0.12);

      osc.connect(gain);
      gain.connect(ctx.destination);

      osc.start(ctx.currentTime + i * 0.04);
      osc.stop(ctx.currentTime + i * 0.04 + 0.13);
    });
  } catch (e) {}
}

// 4. Ringtone Chime Loop
